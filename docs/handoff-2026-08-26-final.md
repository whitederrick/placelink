...
- 통합 기능과 로그인 차단은 `main`의 병합 커밋 `d5d8d34`까지 운영 배포되었다.
- 운영 DB의 다일 코스 마이그레이션과 `place-link.com` 도메인 연결을 완료했다.
- 실운영 전 사고 방지를 위한 로그인 차단 코드는 PR #8의 필수 CI를 모두 통과해
  `main`에 병합됐고, 자동 Production 배포까지 완료됐다.
...
1. `KMA_SERVICE_KEY`를 등록하고 실제 기상청 응답을 확인한다. 미등록 상태에서도
   시간대 기반 대체 문구로 동작한다.
2. 실제 앱 사용자 ID가 확정되면 `ADMIN_USER_IDS`를 등록하고 관리자 화면을
   확인한다.
3. 로그인 재개 전 Kakao/Google OAuth 콜백 URL과 운영 정책을 최종 점검한다.
4. 행사 날짜와 데모/시드 데이터를 운영 기준으로 정리하고 핵심 모바일 흐름을
   최종 QA한다.
