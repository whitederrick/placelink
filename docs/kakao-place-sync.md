...
DB 쓰기는 사람 검토 결과가 코드에 등록된 `--approval-set`을 반드시 요구한다. 승인
목록에 없는 후보는 쓰지 않으며, 승인 당시 후보가 Kakao 검색 결과에서 사라졌다면
일부만 조용히 반영하지 않고 실행을 중단한다.
...
액티비티는 기본적으로 `산책 명소`를 검색하되 연남은 `경의선숲길`, 망원은
`산책`을 사용한다. `여행 > 관광,명소`와 `여행 > 공원`만 허용하고, 같은 이름
또는 괄호 안 방향만 다른 동선은 한 후보로 정리한다.
...
- dry-run → 사람 검토 → 승인 목록 등록 → 승인 목록을 지정한 쓰기 순서를 지킨다.
