...
export const placeAreaSchema = z.enum(["seongsu", "yeonnam", "seochon", "hannam", "mangwon"]);
export const placeCategorySchema = z.enum(["EXHIBITION", "CAFE", "SHOP", "RESTAURANT", "ACTIVITY", "BAR"]);
...
export const mapPlacesQuerySchema = z.object({
  ...
  if (bounds.south >= bounds.north) context.addIssue({ code: "custom", message: "south must be below north", path: ["south"] });
  if (bounds.west >= bounds.east) context.addIssue({ code: "custom", message: "west must be left of east", path: ["west"] });
  if (bounds.north - bounds.south > 1 || bounds.east - bounds.west > 1) {
    context.addIssue({ code: "custom", message: "map viewport is too large", path: ["north"] });
  }
});
