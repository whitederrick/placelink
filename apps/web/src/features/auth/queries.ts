...
export async function selectUserByIdentity(provider: AuthenticationProfile["provider"], externalId: string) {
  return getDatabase().authIdentity.findUnique({
    where: { provider_externalId: { provider, externalId } },
    select: { user: { select: { id: true, nickname: true, status: true } } }
  });
}
...
export async function insertAuthenticatedUser(profile: AuthenticationProfile) {
  return getDatabase().user.create({
    data: {
      ...
      authIdentities: { create: { provider: profile.provider, externalId: profile.externalId } }
    },
