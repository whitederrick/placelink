import { homeFeedSchema, type HomeFeed, type HomeFeedLocale, type HomeFeedQuery } from "./schema";
...
function formatHappeningBadge(status: "UPCOMING" | "ACTIVE" | "ENDED", startsAt: Date, endsAt: Date, now: Date): string {
  if (status === "UPCOMING") return `${formatDate(startsAt)} OPEN`;
  if (status === "ENDED") return "ENDED";
  const daysRemaining = Math.max(0, Math.ceil((endsAt.getTime() - now.getTime()) / MILLISECONDS_PER_DAY));
  return daysRemaining <= 7 ? `D-${daysRemaining}` : `~${formatDate(endsAt)}`;
}

function selectNeighborhood(name: string | undefined): string {
  return name?.split(" ")[0] ?? "SEOUL";
}

export function buildHomeFeed(records: HomeFeedRecords, locale: HomeFeedLocale, now: Date): HomeFeed {
  return homeFeedSchema.parse({
    happenings: records.happenings.map((happening, index) => ({
      id: happening.id,
      neighborhood: selectNeighborhood(happening.place.translations[0]?.name),
      title: happening.translations[0]?.title ?? happening.id,
      period: `${formatDate(happening.startsAt)} — ${formatDate(happening.endsAt)}`,
      dDay: formatHappeningBadge(happening.status, happening.startsAt, happening.endsAt, now),
          ...
          course.couple?.status === "ACTIVE"
            ...
            : course.couple
              ? locale === "ko"
                ? "익명 커플"
                : "Anonymous couple"
              : (course.creatorUser?.nickname ?? (locale === "ko" ? "익명" : "Anonymous")),
        neighborhood: selectNeighborhood(course.nodes[0]?.place.translations[0]?.name),
        duration: formatDuration(course.durationMinutes),
        stops: course._count.nodes,
        scraps: course._count.scraps,
        tags: course.tags.map(({ tag }) => (locale === "ko" ? tag.labelKo : tag.labelEn)),
...
export async function loadHomeFeed(
  ...
  pagination: Pick<HomeFeedQuery, "cursor" | "take"> = { take: DEFAULT_HOME_FEED_LIMIT },
  now = new Date()
