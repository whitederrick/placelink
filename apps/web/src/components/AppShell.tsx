...
export function AppShell({
  ...
  const navigation = navigationItems.map(({ key, path, icon: Icon }) => {
    const href = `/${locale}${path}`;
    const isActive =
      path === "" ? pathname === `/${locale}` : pathname.startsWith(href);
    return { key, href, Icon, isActive };
  });
  ...
  return (
    <div className="site-canvas">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          <nav className="desktop-nav" aria-label={t("label")}>
            {navigation.map(({ key, href, isActive }) => (
              <Link className={isActive ? "active" : ""} href={href} key={key}>
                {t(key)}
              </Link>
            ))}
          </nav>
      ...
      <nav className="tabbar" aria-label={t("label")}>
        {navigation.map(({ key, href, Icon, isActive }) => {
          return (
            <Link
